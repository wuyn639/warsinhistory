...
## Correlation between number of wars and number of battle deaths
...
# not sure how to change the font size of this mosaic plot
...
# need to add caption
# not sure if we need StartYear1
# need more explanations
# not sure why Initiator is not a factor and if it should be fixed
parcoords(dfa6, 
          rownames=FALSE,
          brushMode="1D-axes",
          reorderable=TRUE,
...
[Why do we focus on WWI and WWII specifically?]
From the cleveland dot plot, we see that WWI and WWII are greater in scale than any other war which is not surprising. The natural move is to analyze the effect of those two wars on different indicators such as gdp and education. 
...
## Effects of WWI on the real GDP per capita and Population
```{r}
#read in data
...
p1 <- ggplot(WW1_subset, aes(year, rgdpnapc, color = country),frequency =1) + geom_line() +
    ggtitle("Figure 1a: 1910 -1920 Real GDP Per Capita in 2011 US dollar") +
...
p2 <- ggplot(WW1_subset, aes(year, pop, color = country)) + geom_line() +
    ggtitle("Figure 1b:1910 -1922 Population") +
    scale_x_continuous(breaks=seq(1910, 1922, 1)) +
...
## Effects of WWII on the real GDP per capita and Population
```{r}
...
p1 <- ggplot(WW2_subset, aes(year, rgdpnapc, color = country),frequency =1) + geom_line() +
    ggtitle("Figure 2a:1935 - 1949 Real GDP Per Capita in 2011 US dollar") +
    ...
    annotate("text", x = 1946, y= 0, label = "WWI Ends") + 
    labs (x = "Year", y = "Real GDP Per Capita") +
    theme(legend.title = element_blank(),
          plot.title = element_text(hjust = 0.5, size=16))

p2 <- ggplot(WW2_subset, aes(year, pop, color = country)) + geom_line() +
    ggtitle("Figure 2b:1935 - 1949 Population") +
    scale_x_continuous(breaks=seq(1935, 1949, 1)) +
    scale_color_discrete(breaks=c("Former USSR","United States", "Japan", "Germany", 
                                  "United Kingdom", "France")) +
    geom_vline(xintercept = 1939) +
    annotate("text", x = 1938,y = 0, label = "WWII Begins" )+ 
    geom_vline(xintercept = 1945) +
...
The above two are plots of military expenditure trends of countries involved in World War 2, similar to the one for World War 1. First plot shows the expenditure from 1820s - 2006 and the second shows near to the time period of world war 2. We can see sharp increase in military expenditures of certain countries such as Italy, France, etc. There are some lines which are no continuous. These correspond to missing data. Certain military expenditures data are not present in the dataset used. 
...
To explore the effects of world war 2 over children's education, the above graph was plotted. In contrast with the graph World War 1, this graph concerning countries involved in World War 2 does not show clear and significant patterns.Certain countries like china and Canada saw an increase in primary school enrolment whereas some other countries like Australia and Italy saw a decrease.
...
Similar to the results observed in the graph of HDI for countries involved in World War 1, here as well there are no observable or clear patterns to the changes in HDI for the time period of World War 2. 
